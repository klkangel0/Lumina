...
const EditRoleModal = ({ isOpen, onClose, role, permissions, modules, onSave }) => {
    ...
    useEffect(() => {
        if (role && isOpen) {
            ...
            setColor(role.color || '#6c757d');
            setActive(role.active !== undefined ? role.active : true);

            // Build permissions state: { moduleId: { canView, canCreate, canEdit, canDelete } }
            const permMap = {};
            modules.forEach(mod => {
                const p = permissions.find(pm => pm.moduleId === mod.id) || {};
                permMap[mod.id] = {
                    canView: !!p.canView,
    ...
    }, [role, permissions, modules, isOpen]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await onSave({
                ...
                color,
                active,
                permissions: isAdmin ? undefined : permissionsArray,
            });
        } finally {
            setSaving(false);
        }
                        ...
                        <div className="px-6 py-4">
                            ...
                            {isAdmin ? (
                                <div className="flex flex-col items-center gap-3 py-8 text-slate-500">
                                    ...
                                    <h4 className="text-base font-bold text-slate-700">Permisos Protegidos</h4>
                                    <p className="text-sm text-center max-w-md">
                                        Los permisos del rol Administrador están bloqueados para evitar cambios accidentales que puedan afectar el funcionamiento del sistema.
                                    </p>
                                </div>
                            ) : (
